...
def RowsetToDataframe(rowset, maxRows, columnList=None):
    ...
    if columnList is None:
        ...
        columnList = [column if type(column) !=str else rowset.Schema.IndexOf(column) for column in columnList]
        # check column uniqueness in columnList
        ...
        labels = [rowset.Schema[columnIndex].Name for columnIndex in columnList]
    ...
    for row in rowset:
        if len(rowList) < maxRows:
            if columnList is None:
                ...
                rowData = [row[columnIndex] for columnIndex in columnList]
